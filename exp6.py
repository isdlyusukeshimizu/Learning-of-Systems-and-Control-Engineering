...
def create_nn(input_dim, init_range=0.1):
    ...
    model = Sequential()
    model.add(Dense(mid_unit, input_dim=input_dim, activation='relu',  # 活性化関数をReLUに変更
                    kernel_initializer=initializer, bias_initializer=initializer))  # 中間層
    model.add(Dense(1, activation='linear', kernel_initializer=initializer,
                    bias_initializer=initializer))  # 出力層
    model.compile(optimizer=Adam(learning_rate=learning_rate), loss='mse')  # 最適化アルゴリズムと損失関数の設定
    return model
...
for k in range(3, num_samples):
    ...
    history = nn_model.fit(input_data, np.array([errors[k]]), epochs=1, batch_size=1, verbose=0)  # エポック数とバッチサイズの調整
